...
def compute_class_level_concepts(concept_labels_matrix, uncertainty_matrix, image_labels_matrix):
    ...
    for class_id in range(num_classes):
        ...
        if uncertainty_matrix is not None:
            ...
            # If no uncertainty matrix provided, all concepts are considered visible
            visible_mask = np.ones_like(class_concepts, dtype=bool)
...
def select_common_concepts(class_level_concepts, min_class_count, CUB=True):
    if CUB:
